module tb_adder32_cla;
    reg  [31:0] a, b;
    reg         cin;
    wire [31:0] sum;
    wire        cout;

    adder32_cla uut(.a(a), .b(b), .cin(cin), .sum(sum), .cout(cout));
    ...
    initial begin
        ...
        $display("Test2: %h + %h = %h (cout=%b) expected=00000000 cout=1", a, b, sum, cout);

        a = 32'hDEADBEEF; b = 32'h12345678; #10;
        $display("Test3: %h + %h = %h (cout=%b) expected=F0E21567", a, b, sum, cout);

        a = 32'hAAAAAAAA; b = 32'h55555555; #10;
        $display("Test4: %h + %h = %h (cout=%b) expected=FFFFFFFF", a, b, sum, cout);
